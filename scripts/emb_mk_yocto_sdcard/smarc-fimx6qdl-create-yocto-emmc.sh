...
check_board()
{
	if grep -q "i.MX6Q" /sys/devices/soc0/soc_id; then
		BOARD=smarc-fimx6q
		DTB_PREFIX=imx6q-smarc
		BLOCK=mmcblk3

		if [[ $DISPLAY != "-wvga" && $DISPLAY != "-wxga" && \
		      $DISPLAY != "-xga" && $DISPLAY != "-1080p" && \
		      $DISPLAY != "" ]]; then
			red_bold_echo "ERROR: invalid display, should be lvds, hdmi, dual-display or empty"
                        exit 1
                fi
	elif grep -q "i.MX6QP" /sys/devices/soc0/soc_id; then
		BOARD=smarc-fimx6qp
		DTB_PREFIX=imx6qp-smarc
		BLOCK=mmcblk3
	...
	elif grep -q "i.MX6DL" /sys/devices/soc0/soc_id; then
		BOARD=smarc-fimx6q
		DTB_PREFIX=imx6dl-smarc
		BLOCK=mmcblk3
	...
	elif grep -q "i.MX6S" /sys/devices/soc0/soc_id; then
		BOARD=smarc-fimx6s
		DTB_PREFIX=imx6dl-smarc
		BLOCK=mmcblk3
	else
...
install_kernel_to_emmc()
{
	...
	cp -v ${IMGS_PATH}/${DTB_PREFIX}${DISPLAY}.dtb ${MOUNTDIR}/dtbs/${DTB_PREFIX}.dtb
...
{
	echo
	echo "This script installs Yocto Rootfs on the SMARC-iMX8MQ's internal eMMC storage device"
	echo
...
blue_underlined_bold_echo "*** Embedian SMARC-iMX8MQ on-module eMMC Recovery ***"
echo
