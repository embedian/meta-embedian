...
check_board()
{
	if grep -q "i.MX8MQ" /sys/devices/soc0/soc_id; then
		...
		if [[ $DISPLAY != "-dcss-lvds" && $DISPLAY != "-hdmi" && \
		      $DISPLAY != "-lcdif-lvds" && $DISPLAY != "-dp" && \
		      $DISPLAY != "-dual-display" && $DISPLAY != "" ]]; then
			red_bold_echo "ERROR: invalid display, should be dcss-lvds, hdmi, lcdif-lvds, dp, dual-display or empty"
			exit 1
		fi
	else
...
usage()
{
	...
	echo " -d <dcss-lvds|lcdif-lvds|hdmi|dp|dual-display>  set display type, default is hdmi"
	echo
}
