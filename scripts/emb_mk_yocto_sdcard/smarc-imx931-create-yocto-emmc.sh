...
check_images()
{
	if [[ ! -f $IMGS_PATH/$UBOOT_IMAGE ]] ; then
        ...
        if [[ ! -f $IMGS_PATH/${DTB_PREFIX}-1g.dtb ]] ; then
                red_bold_echo "ERROR: \"$IMGS_PATH/${DTB_PREFIX}-1g.dtb\" does not exist"
                exit 1
        fi
...
install_kernel_to_emmc()
{
	...
	cp -v ${IMGS_PATH}/${DTB_PREFIX}-1g.dtb ${MOUNTDIR}/dtbs/imx93-smarc.dtb
...
install_rootfs_to_emmc()
{
	...
	tar --warning=no-timestamp -jxvf ${IMGS_PATH}/${ROOTFS_IMAGE} -C ${MOUNTDIR} --checkpoint=.1200
                ...
                # Create DTB symlink
                (cd ${MOUNTDIR}/${BOOTDIR}; ln -fs ${DTB_PREFIX}-1g.dtb ${DTB_PREFIX}.dtb)
...
{
	echo
	echo "This script installs Yocto Rootfs on the SMARC-iMX93's internal eMMC storage device"
	echo
...
blue_underlined_bold_echo "*** Embedian SMARC-iMX93 on-module eMMC Recovery ***"
echo
