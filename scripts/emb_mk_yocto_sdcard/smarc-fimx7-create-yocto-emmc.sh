...
check_board()
{
	if grep -q "i.MX7D" /sys/devices/soc0/soc_id; then
		BOARD=smarc-fimx7d
		DTB_PREFIX=imx7d-smarc
		BLOCK=mmcblk2
...
check_images()
{
	if [[ ! -f $IMGS_PATH/$UBOOT_IMAGE ]] ; then
        ...
        if [[ ! -f $IMGS_PATH/${DTB_PREFIX}.dtb ]] ; then
                red_bold_echo "ERROR: \"$IMGS_PATH/${DTB_PREFIX}.dtb\" does not exist"
                exit 1
        fi
...
install_kernel_to_emmc()
{
	...
	cp -v ${IMGS_PATH}/${DTB_PREFIX}.dtb ${MOUNTDIR}/dtbs/${DTB_PREFIX}.dtb
...
install_rootfs_to_emmc()
{
	...
	tar --warning=no-timestamp -jxvf ${IMGS_PATH}/${ROOTFS_IMAGE} -C ${MOUNTDIR} --checkpoint=.1200
                ...
                # Create DTB symlink
                (cd ${MOUNTDIR}/${BOOTDIR}; ln -fs ${DTB_PREFIX}.dtb ${DTB_PREFIX}.dtb)
