...
check_board()
{
	if grep -q "i.MX8QM" /sys/devices/soc0/soc_id; then
		BOARD=smarc-imx8qm
		DTB_PREFIX=imx8qm-smarc
		BLOCK=mmcblk0

		if [[ $DISPLAY != "-lvds" && $DISPLAY != "-hdmi" && \
		      $DISPLAY != "-edp" && $DISPLAY != "-dp" && $DISPLAY != "" ]]; then
			red_bold_echo "ERROR: invalid display, should be lvds, hdmi, dp, edp or empty"
			exit 1
		fi
	else
...
{
	echo
	echo "This script installs Yocto Rootfs on the SMARC-iMX8QM's internal eMMC storage device"
	echo
	echo " Usage: $(basename $0) <option>"
	echo
	echo " options:"
	echo " -h                           show help message"
	echo " -d <lvds|hdmi|dp|edp>  set display type, default is lvds"
	echo
}
...
blue_underlined_bold_echo "*** Embedian SMARC-iMX8QM on-module eMMC Recovery ***"
echo
